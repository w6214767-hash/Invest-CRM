...
def test_profile_is_created_and_updated() -> None:
    with session() as db:
        ...
        updated = update_profile(SearchProfilePayload(name="Новый профиль", districts=["Чехов"], min_price=1, max_price=2, min_area=1, max_area=2, min_discount=10, land_use=["ИЖС"], exclude_words=[]), db)

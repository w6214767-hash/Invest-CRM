...
export default function Login() {
  ...
  return <main className="login-page"><form className="login-card" onSubmit={submit}><p className="eyebrow">ЮРЖИЛСЕРВИС</p><h1>Invest-CRM</h1><p>Войдите в систему поиска и оценки инвестиционных участков.</p><label>Email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></label><label>Пароль<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required /></label>{error && <div className="notice">{error}</div>}<button className="button" type="submit">Войти</button></form></main>
}

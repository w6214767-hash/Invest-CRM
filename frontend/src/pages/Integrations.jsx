...
export default function Integrations() {
  ...
  const tabs = [
    ...
    ['review', `Ручная проверка · ${reviewListings.length}`],
  ...
  return <section className="integrations-page">
    ...
    <div className="integration-summary">
      {[
        ['↗', '210', 'объявлений сегодня'],
        ['✦', '32', 'прошли ИИ-скоринг'],
        ['◎', '14', 'переданы в воронку'],
        ['₽', '4 860 ₽', 'стоимость лида']
    ...
    {activeTab === 'sources' && <>
      <div className="section-title"><div><p className="eyebrow">ИСТОЧНИКИ</p><h2>Площадки объявлений</h2></div><span>2 из 3 подключены</span></div>
    ...
    {activeTab === 'review' && <section className="workspace-panel"><div className="section-title"><div><p className="eyebrow">HUMAN IN THE LOOP</p><h2>Очередь ручной проверки</h2></div><span>Решение принимает менеджер</span></div><div className="review-list">{reviewListings.map((listing) => <article key={listing.id}><span className="risk-icon">!</span><div><Link to={`/listings/${listing.id}`}>{listing.title}</Link><p>{listing.red_flags.join(' · ') || 'Не указан кадастровый номер'}</p></div><strong>{listing.score.toFixed(1)}</strong><Link className="review-action" to={`/listings/${listing.id}`}>Проверить →</Link></article>)}</div></section>}
    ...
    {activeTab === 'audit' && <section className="workspace-panel"><div className="section-title"><div><p className="eyebrow">ПРОЗРАЧНОСТЬ</p><h2>Журнал решений агентов</h2></div><button className="export-button" onClick={() => setMessage('Экспорт журнала подготовлен.')}>Экспорт CSV</button></div><div className="audit-list">{auditEvents.map((event) => <article key={`${event.time}-${event.action}`}><time>{event.time}</time><i className={event.status} /><div><strong>{event.action}</strong><p>{event.agent} · {event.reason}</p></div><button aria-label="Подробнее">•••</button></article>)}</div></section>}
  </section>
}

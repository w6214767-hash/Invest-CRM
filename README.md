...
make browser-setup # установить Chromium и системные библиотеки для Playwright
```

Команда `make browser-setup` предназначена для Ubuntu/Debian и требует прав на
установку системных пакетов. После неё можно запускать визуальные проверки и
создавать скриншоты интерфейса через Playwright без ручного поиска библиотек.

# ИнвестСкан — самостоятельный инвестиционный модуль

Поиск и обработка объектов для срочного выкупа. Отдельные React/FastAPI-приложение, база данных и инфраструктура; стиль и возврат в Штаб Юржилсервиса. Без интеграции Bitrix24.

**Версия 0.2 в разработке.** Реализованы рабочие карточки, импорт, история цен, расчёт выкупа, проверки, задачи, покупатели и воронка. Внешний сбор объявлений, связь с работающим Штабом и боевой VPS ещё не подключены.

- [Что реализовано, локальный запуск, SSO и VPS](docs/investscan-standalone.md)
- [Отдельный Docker Compose](deploy/investscan/compose.yml)
- API: `/api/v1/investscan`.
- Штаб: `https://shtab.ugsdom.ru/#platform`.

Старые CRM-таблицы сохранены для совместимости. Новое приложение использует отдельные таблицы `is_*`; старые бизнес-маршруты и регистрация доступны только в development.

<details>
<summary>Историческое описание каркаса Invest-CRM (до версии 0.2)</summary>

# Invest-CRM

Open-source скелет инвестиционной **Invest-CRM** для строительной компании **ЮрЖилСервис** (Домодедово, Московская область). Проект помогает находить земельные участки на Авито ниже рыночной цены, оценивать их, вести полуавтоматические переговоры и передавать подтверждённые сделки в Bitrix24.
...
- Постоянные профили поиска, очередь ручной проверки и аудит запусков через API `/api/v1/integrations`.
